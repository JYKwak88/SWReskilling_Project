...
{
	while((SPIx->SR&SPI_I2S_FLAG_TXE)==RESET);		//송신 영역이 비어 있을 때까지 기다리는 중	  
	SPIx->DR=Byte;	 	//바이트 보내기   
	while((SPIx->SR&SPI_I2S_FLAG_RXNE)==RESET);//1바이트가 수신될 때까지 기다립니다.  
	return SPIx->DR;          	     //받은 데이터를 반환			
} 
